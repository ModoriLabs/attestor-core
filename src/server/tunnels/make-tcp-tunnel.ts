 ...
 */
export const makeTcpTunnel: MakeTunnelFn<
  ExtraOpts,
  TCPSocketProperties
> = async ({ onClose, onMessage, logger, ...opts }) => {
  ...
  try {
    await new Promise(async (resolve, reject) => {
      try {
        ...
        socket.once("end", () =>
          reject(new AttestorError("ERROR_NETWORK_ERROR", "connection closed"))
        )
      } catch (err) {
        reject(err)
      }
...
async function _getSocket({ host, port, geoLocation, logger }: ExtraOpts) {
