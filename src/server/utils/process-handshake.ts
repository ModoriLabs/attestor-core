...
export async function processHandshake(
  receipt: ClaimTunnelRequest["transcript"],
  logger: Logger
) {
  ...
  if (
    tlsVersion === "TLS1_2" &&
    (serverChangeCipherSpecMsgIdx < 0 || clientChangeCipherSpecMsgIdx < 0)
  ) {
  ...
  async function readPacket(getMoreData = false) {
    ...
    if (!handshakeData?.length || getMoreData) {
      ...
      if (
        ...
        const { plaintext } = await decryptDirect(
          reveal?.directReveal,
          cipherSuite,
          recordHeader,
          tlsVersion,
