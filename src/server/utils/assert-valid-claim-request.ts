...
export async function assertValidClaimRequest(
  ...
  const serialisedReq = ClaimTunnelRequest.encode({
    ...request,
...
export async function assertValidProviderTranscript<
  T extends ProviderClaimInfo,
>(applData: Transcript<Uint8Array>, info: T, logger: Logger) {
...
export async function decryptTranscript(
  ...
  const { tlsVersion, cipherSuite, hostname, nextMsgIndex } =
  ...
  for (const [
    i,
    { sender, message, reveal: { zkReveal, directReveal } = {} },
