...
const dateHeaderRegex =
  "[dD]ate: ((?:Mon|Tue|Wed|Thu|Fri|Sat|Sun), (?:[0-3][0-9]) (?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) (?:[0-9]{4}) (?:[01][0-9]|2[0-3])(?::[0-5][0-9]){2} GMT)"
const dateDiff = 1000 * 60 * 10 // allow 10 min difference

type HTTPProviderParams = ProviderParams<"http">

type HTTPResponseRedaction = Exclude<
  HTTPProviderParams["responseRedactions"],
  undefined
>[number]
...
const HTTP_PROVIDER: Provider<"http"> = {
  ...
  geoLocation(params) {
    return "geoLocation" in params ? getGeoLocation(params) : undefined
  },
  ...
  createRequest(secretParams, params, logger) {
    ...
    const hasUserAgent =
      Object.keys(pubHeaders).some(k => k.toLowerCase() === "user-agent") ||
      Object.keys(secHeaders).some(k => k.toLowerCase() === "user-agent")
    if (!hasUserAgent) {
      ...
      for (const hiddenBodyPart of newParams.hiddenBodyParts) {
        if (hiddenBodyPart.length) {
          redactions.push({
            ...
            toIndex:
              headerStr.length + hiddenBodyPart.index + hiddenBodyPart.length,
          })
        }
      }
    }
  ...
  assertValidProviderReceipt(receipt, paramsAny, logger) {
    ...
    const secretParams =
      "secretParams" in paramsAny
        ? (paramsAny.secretParams as ProviderSecretParams<"http">)
        : undefined
    const newParams = substituteParamValues(
      paramsAny,
      secretParams,
      !secretParams
    )
    ...
    const expectedPath =
      pathname.replaceAll("%7B", "{").replaceAll("%7D", "}") +
    ...
    if (req.headers.host !== expectedHostStr) {
      throw new Error(
        `Expected host: ${expectedHostStr}, found: ${req.headers.host}`
      )
    }
    ...
    if (connectionHeader !== "close") {
      throw new Error(
        `Connection header must be "close", got "${connectionHeader}"`
      )
    }
    ...
    if (dateHeader?.length > 1) {
      ...
      if (Date.now() - serverDate.getTime() > dateDiff) {
        logger.info(
          { dateHeader: dateHeader[0], current: Date.now() },
          "date header is off"
        )
    ...
    for (const { type, value, invert } of params.responseMatches || []) {
      ...
      switch (type) {
        case "regex":
          ...
          if (match === inv) {
            // if both true or both false then fail
            throw new Error(
              "Invalid receipt." +
    ...
    function logTranscript() {
      ...
      logger.debug({
        request: clientTranscript,
        response: serverTranscript,
        params: paramsAny,
      })
    }
  },
...
function substituteParamValues(
  ...
  const urlParams = extractAndReplaceTemplateValues(
    params.url,
    ignoreMissingParams
  )
  if (urlParams) {
    ...
    if (urlParams.hiddenParts.length) {
      ...
      for (const hiddenURLPart of urlParams.hiddenParts) {
        hiddenURLParts.push({
          index: hiddenURLPart.index - offset,
          length: hiddenURLPart.length,
        })
      }
    }
  }
    ...
    param = param.replace(paramsRegex, (match, pn, offset) => {
      if (params.paramValues && pn in params.paramValues) {
        ...
        if (secretParams?.paramValues && pn in secretParams?.paramValues) {
          ...
          throw new Error(
            `parameter's "${pn}" value not found in paramValues and secret parameter's paramValues`
          )
        }
      } else {
...
function getURL(
  ...
  for (const pn of paramNames) {
    if (v2Params.paramValues && pn in v2Params.paramValues) {
      hostPort = hostPort?.replaceAll(
        `{{${pn}}}`,
        v2Params.paramValues[pn].toString()
      )
    } else if (secretParams?.paramValues && pn in secretParams.paramValues) {
      hostPort = hostPort?.replaceAll(
        `{{${pn}}}`,
        secretParams.paramValues[pn].toString()
      )
    } else {
      throw new Error(`parameter "${pn}" value not found in templateParams`)
    }
