...
export function extractHTMLElement(
  ...
  const { start, end } = extractHTMLElementIndex(
    html,
    xpathExpression,
    contentsOnly
  )
...
export function extractHTMLElements(
  ...
  const indexes = extractHTMLElementsIndexes(
    html,
    xpathExpression,
    contentsOnly
  )
...
export function extractHTMLElementsIndexes(
  ...
  for (const node of nodes) {
    ...
    if (!nodeLocation) {
      throw new Error(
        `Failed to find XPath node location: "${xpathExpression}"`
      )
    }

    if (contentsOnly) {
      const start = nodeLocation.startTag
        ? nodeLocation.startTag.endOffset
        : nodeLocation.startOffset
      const end = nodeLocation.endTag
        ? nodeLocation.endTag.startOffset
        : nodeLocation.endOffset
      res.push({ start, end })
    } else {
...
export function extractJSONValueIndexes(
  json: string,
  jsonPath: string
): { start: number; end: number }[] {
...
function traverse(
  ...
  if (o instanceof ObjectExpression) {
    for (const p of o.properties) {
      ...
      const localPath =
        p.key.type === Syntax.Literal ? path + "/" + p.key.value : path
 ...
 * @param chunks
 */
export function convertResponsePosToAbsolutePos(
  pos: number,
  bodyStartIdx: number,
  chunks?: ArraySlice[]
): number {
 ...
 */
export function matchRedactedStrings(
  templateString: Uint8Array,
  redactedString?: Uint8Array
): boolean {
...
export function generateRequstAndResponseFromTranscript(
  ...
  const responsePackets = concatenateUint8Arrays(
    packets
      .filter(p => p.sender === "server")
      .map(p => p.message)
