...
async function main() {
  ...
  for (const folder of folders) {
    ...
    } = await generateTsFromYamlSchema(folder, "secret-parameters")
...
export async function generateTsFromYamlSchema(
  ...
  let paramsSchemaTs = await compile(paramsJson, "", {
...
async function findAllProviderFolders() {
  const providerFolders = await readdir(PROVIDER_SCHEMAS_PATH, {
    withFileTypes: true,
  })
