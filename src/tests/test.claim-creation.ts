...
const TLS_VERSIONS: TLSProtocolVersion[] = ["TLS1_3", "TLS1_2"]
...
jest.mock("@reclaimprotocol/tls/lib/utils/parse-certificate", () => {
  const actual = jest.requireActual(
    "@reclaimprotocol/tls/lib/utils/parse-certificate"
  )
  return {
    __esModule: true,
    ...actual,
...
describeWithServer("Claim Creation", opts => {
  ...
  it.each(TLS_VERSIONS)(
    "should successfully create a claim (%s)",
    async version => {
      providers.http.additionalClientOptions = {
        ...providers.http.additionalClientOptions,
  ...
  describe("OPRF via %s", () => {
    ...
    it.each(OPRF_CIPHER_SUITES)(
      "should create a claim with an OPRF redaction (%s)",
      async cipherSuite => {
  ...
  describe("Pool", () => {
    ...
    it("should retry on network errors", async () => {
      ...
      client.sendMessage = async () => {
        ...
        const err = new AttestorError("ERROR_NETWORK_ERROR", "F")
