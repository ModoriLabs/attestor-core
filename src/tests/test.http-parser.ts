...
describe.each(["complete", "byte-by-byte"] as const)(
  "HTTP Parser tests (mode=%s)",
  parseMode => {
    it("should parse a response", () => {
      const res = parseHttpResponse(RES1, parseMode)
      expect(RES1.subarray(0, res.statusLineEndIndex).toString()).toEqual(
    ...
    it("should parse an empty body response", () => {
      const res = parseHttpResponse(strToUint8Array(RES_EMPTY), parseMode)
      expect(res.complete).toEqual(true)
    ...
    it("should correctly set chunk indices", () => {
      ...
      const parsedChunks = res.chunks?.map(chunk => {
        return uint8ArrayToStr(buff.slice(chunk.fromIndex, chunk.toIndex))
      })
