...
import {
  ...
  WindowRPCOutgoingMsg,
  WindowRPCResponse,
} from "src/window-rpc/types"
import {
  generateRpcRequestId,
  getCurrentMemoryUsage,
  getWsApiUrlFromLocation,
  mapToCreateClaimResponse,
  waitForResponse,
} from "src/window-rpc/utils"
import {
  ALL_ENC_ALGORITHMS,
  makeWindowRpcOprfOperator,
  makeWindowRpcZkOperator,
} from "src/window-rpc/window-rpc-zk"
...
const VALID_MODULES = ["attestor-core", "witness-sdk"]
...
export function setupWindowRpc() {
  ...
  async function handleMessage(event: MessageEvent<any>) {
    ...
    function respond<K extends keyof WindowRPCClient>(
      data: WindowRPCResponse<WindowRPCClient, K> | WindowRPCErrorResponse
    ) {
      const res = {
        ...data,
        id,
    ...
    async function updateProviderParams(
      ...
      bridge.send({
        ...
        request: {
          request: {
            ...req,
            body: req.body ? uint8ArrayToStr(req.body) : undefined,
          },
