...
export function makeBarretenbergZKOperator({
	...
	let concurrencyLimiter: any

	async function getConcurrencyLimiter() {
		if (!concurrencyLimiter) {
			const PQueue = (await import('p-queue')).default
			concurrencyLimiter = new PQueue({ concurrency: maxProofConcurrency })
		}
	...
	return {
		...
		async groth16Prove(witness: Uint8Array, logger?: Logger): Promise<{ proof: Uint8Array }> {
			...
			const limiter = await getConcurrencyLimiter()
