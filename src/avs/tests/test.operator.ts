...
describe("Operators", () => {
  ...
  beforeAll(async () => {
    ...
    createClaimFn.mockImplementation(
      async ({
        ownerPrivateKey,
        name,
        params,
        ...
        const op = operators.find(op => op.url === client.url.toString())
        if (!op) {
  ...
  async function registerFirstOperator() {
    ...
    assert.strictEqual(
      await contracts.registryContract.operatorRegistered(operatorAddress),
      true
    )
  ...
  async function registerSecondOperator() {
    ...
    const meta = await contracts.contract.getMetadataForOperator(newAddr)
  ...
  async function createNewTask(userWallet: Wallet, claimOwner = userWallet) {
  ...
  async function markTaskAsCompleted(
    ...
    assert.ok(task.operators.length > 0, "No operators selected for the task")
    ...
    for (const { wallet: operator } of operators) {
      ...
      const selectedOp = task.operators.some(op => op.addr === opAddr)
      if (!selectedOp) {
        continue
      }

      const signature = await operator.signMessage(signData)
...
function randomWallet() {
  return Wallet.createRandom().connect(contracts.provider)
}
