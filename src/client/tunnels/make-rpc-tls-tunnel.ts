 ...
 */
export const makeRpcTlsTunnel: MakeTunnelFn<
  ExtraTLSOptions,
  TLSTunnelProperties
> = async ({ onMessage, onClose, tlsOpts, request, connect, logger }) => {
  ...
  const tls = makeTLSClient({
    ...
    async write(packet, ctx) {
      const message = concatenateUint8Arrays([packet.header, packet.content])
      transcript.push({
        sender: "client",
