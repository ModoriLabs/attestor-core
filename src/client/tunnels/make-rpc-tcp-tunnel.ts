...
export const makeRpcTcpTunnel: MakeTunnelFn<TCPTunnelCreateOpts> = ({
  ...
  client.addEventListener(
    ...
    onConnectionTerminatedListener
  )
  ...
  function onDisconnectListener({ data }: RPCEvent<"tunnel-disconnect-event">) {
    ...
    onErrorRecv(
      data.error?.code ? AttestorError.fromProto(data.error) : undefined
    )
  }

  function onConnectionTerminatedListener({
    data,
  }: RPCEvent<"connection-terminated">) {
    onErrorRecv(data)
  }
