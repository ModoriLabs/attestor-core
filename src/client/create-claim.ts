...
async function _createClaimOnAttestor<N extends ProviderName>({
  ...
  const requestData =
    typeof requestStr === "string" ? strToUint8Array(requestStr) : requestStr

  logger.debug({ redactions: redactions.length }, "generated request")

  const waitForAllData = new Promise<void>((resolve, reject) => {
  ...
  if (updateProviderParams) {
    const { params: updatedParms, secretParams: updatedSecretParms } =
  ...
  const requestSignature = await signatureAlg.sign(
    claimTunnelBytes,
    ownerPrivateKey
  )
  ...
  async function writeRedactedWithKeyUpdate() {
    ...
    for (const section of redactions) {
      const block = requestData.slice(currentIndex, section.fromIndex)
      if (block.length) {
  ...
  async function writeWithReveal(data: Uint8Array, reveal: boolean) {
    ...
    setRevealOfLastSentBlocks(
      reveal ? { type: "complete" } : undefined,
      blocksWritten
    )
  ...
  async function addServerSideReveals() {
    ...
    if (provider.getResponseRedactions) {
      serverPacketsToReveal = await getBlocksToReveal(
        serverBlocks,
        total => provider.getResponseRedactions!(total, params, logger),
        performOprf
      )
    }

    const revealedPackets: Transcript<Uint8Array> = packets.filter(
      p => p.sender === "client"
    )
