...
export class AttestorClient extends AttestorSocket implements IAttestorClient {
  ...
  constructor({
    ...
    super(makeWebSocket(url) as WebSocket, initRequest, logger)

    const initReqId = msg.messages[0].id
    this.waitForInitPromise = this.waitForResponse<"init">(initReqId).then(
      res => {
  ...
  async rpc<T extends RPCType>(type: T, request: Partial<RPCRequestData<T>>) {
    const {
      messages: [{ id }],
