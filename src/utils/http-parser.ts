...
export function makeHttpResponseParser() {
  ...
  let isChunked = false
  let remaining: Uint8Array<ArrayBufferLike> = new Uint8Array()
        ...
        for (
          ...
          if (!res.statusCode) {
            const [, statusCode, statusMessage] =
              line.match(/HTTP\/\d\.\d (\d+) (.*)/) || []
            res.statusCode = Number(statusCode)
            res.statusMessage = statusMessage
            res.statusLineEndIndex =
              currentByteIdx - HTTP_HEADER_LINE_END.length
          } else if (line === "") {
            ...
            res.headerIndices[key.toLowerCase()] = {
              fromIndex:
                currentByteIdx - line.length - HTTP_HEADER_LINE_END.length,
 ...
 */
export function getHttpRequestDataFromTranscript(
